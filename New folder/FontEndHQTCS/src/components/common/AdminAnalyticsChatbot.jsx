...
import { chatWithAnalyticsAgent, getAnalyticsMeta } from '../../api/analyticsAgentService'
import { getApiErrorMessage } from '../../api/client'
import { buildDatamartDateRange } from '../../utils/dateUtils'

function buildQuickPrompts(latestSalesDate, earliestSalesDate) {
  const { start, end } = buildDatamartDateRange(latestSalesDate, 30)

  return [
    'Doanh thu hôm nay',
    'Doanh thu tháng này',
    'Doanh thu năm nay',
    earliestSalesDate && earliestSalesDate < end
      ? `Tóm tắt doanh thu toàn bộ từ ${earliestSalesDate} đến ${end}`
      : `Tóm tắt doanh thu từ ${start} đến ${end}`,
    `Thuốc nào bán chạy nhất từ ${start} đến ${end}?`,
    'Tồn kho nào cần chú ý?',
  ]
}

function buildWelcomeMessage(latestSalesDate, earliestSalesDate, datamartDatabase) {
  const base =
    'Xin chào, tôi là trợ lý phân tích. Bạn có thể hỏi về doanh thu, thuốc bán chạy, tồn kho, khách hàng hoặc dự báo doanh thu.'
  if (!latestSalesDate) return base
  const rangeText = earliestSalesDate && earliestSalesDate < latestSalesDate
    ? `từ ${earliestSalesDate} đến ${latestSalesDate}`
    : `đến ${latestSalesDate}`
  return `${base}\n\nDữ liệu datamart (${datamartDatabase || 'HQT_BanChuan'}) ${rangeText}. Bạn có thể hỏi: hôm nay, tháng này, năm nay, "6 tháng qua", "năm 2024", "từ 2025-01-01 đến 2025-12-31", hoặc "toàn bộ".`
}
...
export default function AdminAnalyticsChatbot() {
  ...
  useEffect(() => {
    ...
    getAnalyticsMeta()
      .then((meta) => {
        if (cancelled) return
        const latestSalesDate = meta?.latestSalesDate
        const earliestSalesDate = meta?.earliestSalesDate
        setQuickPrompts(buildQuickPrompts(latestSalesDate, earliestSalesDate))
        setMessages([
          {
            id: 1,
            sender: 'bot',
            text: buildWelcomeMessage(latestSalesDate, earliestSalesDate, meta?.datamartDatabase),
          },
        ])
        setMetaLoaded(true)
      })
