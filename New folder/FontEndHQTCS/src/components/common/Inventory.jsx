...
export default function Inventory() {
  ...
  const mergeMedicine = useCallback(
    (medicine) => {
      ...
      const detail = medicineDetails[medicine.id]
      if (!detail) return medicine
      return { ...medicine, ...detail, batches: detail.batches || medicine.batches || [] }
    },
    [medicineDetails],
  )

  const cacheMedicineDetail = useCallback(async (medicineId) => {
    const detail = await loadMedicineDetail(medicineId)
    if (detail) {
      setMedicineDetails((prev) => ({ ...prev, [medicineId]: detail }))
    }
    return detail
  }, [loadMedicineDetail])
  ...
  useEffect(() => {
    ...
    const keyword = importForm.medicineName.trim()
    if (keyword.length < 2) {
      setImportSearchResults([])
      return undefined
    }
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        setImportSearchResults(result.items.map((item) => mapMedicineToFe(item, [])))
      } catch {
        setImportSearchResults([])
      }
    }, 300)
  ...
  const fetchMedicines = useCallback(
    async (page) => {
      const stockStatus =
        filterStatus === 'Bình thường'
          ? 'normal'
          : filterStatus === 'Sắp hết/Hết hàng'
            ? 'low'
            : undefined
      ...
      const result = await getMedicines({
        ...
        isActive: true,
        stockStatus,
        sortBy: stockSortOrder ? 'stock' : undefined,
        sortOrder: stockSortOrder || undefined,
      })
      return {
        items: result.items.map((item) => mapMedicineToFe(item, [])),
        meta: result.meta,
      }
    },
    [search, filterStatus, stockSortOrder],
  )
  ...
  const {
    ...
    reload,
    meta: medicineMeta,
  } = useServerPagination(fetchMedicines, [search, filterStatus, stockSortOrder])
  ...
  const groupedMedicines = useMemo(() => {
    return medicines.reduce((groups, item) => {
  ...
  const batchDetailMedicine = useMemo(
    () =>
      mergeMedicine(
        medicines.find((medicine) => medicine.id === batchDetailModal.medicineId) ||
          medicineDetails[batchDetailModal.medicineId] ||
          null,
      ),
    [batchDetailModal.medicineId, medicines, medicineDetails, mergeMedicine],
  )
  ...
  const toggleMedicineBatches = async (medicineId) => {
    ...
    if (willExpand && !medicineDetails[medicineId]?.batches?.length) {
      await cacheMedicineDetail(medicineId)
    }
  }

  const openBatchDetailModal = async (medicine) => {
    if (!medicineDetails[medicine.id]?.batches?.length) {
      await cacheMedicineDetail(medicine.id)
    }
    setBatchDetailModal({
      isOpen: true,
  ...
  const openEditMedicineModal = async (medicine) => {
    const detail = (await cacheMedicineDetail(medicine.id)) || mergeMedicine(medicine)
    const nearestBatch = getNearestExpiryBatch(detail) || detail.batches?.[0] || null
    setMedicineForm({
      id: detail.id,
      name: detail.name,
      unit: detail.unit,
      type: detail.type || 'Thuốc không kê đơn',
      category: detail.category || '',
      listPrice: detail.listPrice || detail.salePrice || '',
      minStock: detail.minStock || '',
      supplierName: detail.supplierName || '',
      manufacturerName:
        nearestBatch?.manufacturerName || detail.manufacturerName || '',
      importPrice: nearestBatch?.importPrice ?? detail.lastImportPrice ?? '',
      status: detail.status || 'ACTIVE',
      batchId: nearestBatch?.id || '',
      lotCode: nearestBatch?.lotCode || '',
      lotQty: nearestBatch?.qty ?? detail.stock ?? '',
  ...
  const handleSelectMedicineBatch = (batchId) => {
    const medicine = mergeMedicine(medicines.find((item) => item.id === medicineForm.id) || medicineDetails[medicineForm.id])
  ...
  const submitMedicine = async (e) => {
    ...
    const currentMedicine = mergeMedicine(medicines.find((item) => item.id === id) || medicineDetails[id])
    ...
    const nextBatch = {
      ...
      importDate: currentBatch?.importDate || formatLocalDate(),
      importPrice,
      supplierName,
      manufacturerName:
        manufacturerName ||
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {listLoading ? (
                ...
                Object.entries(groupedMedicines).flatMap(([type, items]) => [
                  ...
                  ...items.flatMap((item) => {
                    const displayItem = mergeMedicine(item)
                    const status = getDisplayStatus(displayItem)
                    const isExpanded = Boolean(expandedMedicineIds[item.id])
                    const batches = displayItem.batches || []
                ...
                <tr>
                  <td colSpan={isAdmin ? 4 : 3} className="p-10 text-center text-slate-400">
                    {filterStatus !== 'Tất cả' || stockSortOrder
                    ...
                    {medicineModal.mode === 'edit' && (
                      <label className="space-y-1 text-sm font-medium text-slate-700 md:col-span-2">
                        ...
                        <select
                          ...
                          className="w-full rounded-xl border border-slate-300 px-3 py-2 outline-none focus:border-emerald-500"
                        >
                          {(mergeMedicine(medicines.find((entry) => entry.id === medicineForm.id) || medicineDetails[medicineForm.id])?.batches || []).map((batch) => (
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <label className="space-y-1 text-sm font-medium text-slate-700 md:col-span-2">
                    ...
                    <datalist id="medicine-name-options">
                      {[...importSearchResults, ...medicines]
                        .filter(
                          (medicine, index, list) =>
                            list.findIndex((entry) => entry.id === medicine.id) === index,
                        )
                        .map((medicine) => (
