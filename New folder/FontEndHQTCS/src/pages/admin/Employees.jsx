...
export default function Employees() {
  ...
  const { updateEmployeeRole, toggleEmployeeStatus } = useInventoryAlerts()
  ...
  const fetchEmployees = useCallback(
    async (page) => {
      const result = await getEmployees({
        ...
        role: filterRole === 'Tất cả' ? undefined : filterRole,
      })
      return {
        items: result.items.map(mapEmployeeToFe),
        meta: result.meta,
      }
    },
    [filterRole, search],
  ...
  const summary = meta.summary || {
    ...
    adminCount: employees.filter((item) => item.role === 'Admin').length,
    activeCount: employees.filter((item) => item.isActive).length,
  }
