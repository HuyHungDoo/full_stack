...
export default function Orders() {
  ...
  const { returnOrderItems, cancelOrder, loadOrderDetail } = useInventoryAlerts()
  ...
  const fetchOrders = useCallback(
    async (page) => {
      const result = await getSalesInvoices({
        ...
        employeeId: isAdmin ? undefined : currentUser?.employeeId,
      })
      return {
        items: result.items.map((invoice) => mapInvoiceToOrder(invoice, [])),
        meta: result.meta,
      }
    },
    [currentUser?.employeeId, isAdmin, search, selectedStatus],
  )
  ...
  const handleExportOrders = async () => {
    ...
    try {
      ...
      const exportOrders = result.items.map((invoice) => mapInvoiceToOrder(invoice, []))
      ...
      const rows = exportOrders.map((item) => [
        item.id,
        item.customerName,
        item.phone || '',
        item.date,
    ...
    window.URL.revokeObjectURL(url)
      ...
      showError(getApiErrorMessage(error, 'Không thể xuất danh sách đơn hàng'))
    }
  }
