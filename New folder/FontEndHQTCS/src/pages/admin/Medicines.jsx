...
export default function Medicines() {
  ...
  const { addMedicine, updateMedicine } = useInventoryAlerts()
  ...
  const fetchMedicines = useCallback(
    async (page) => {
      const result = await getMedicines({
        ...
        categoryName: selectedGroup === ALL_GROUP_OPTION ? undefined : selectedGroup,
        productType: selectedTypes.length === 1 ? selectedTypes[0] : undefined,
        isActive: true,
      })
      return {
    ...
    [search, selectedGroup, selectedTypes],
  )
  ...
  const visibleMedicines = useMemo(() => {
    if (selectedTypes.length <= 1) return medicines
    return medicines.filter((item) => selectedTypes.includes(item.type))
  }, [medicines, selectedTypes])
  ...
  const handleSubmitMedicine = (e) => {
    ...
    const newItem = {
      id: formData.code || `SP${Date.now().toString().slice(-6)}`,
      name: formData.name,
