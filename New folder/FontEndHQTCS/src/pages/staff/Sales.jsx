...
export default function Sales() {
  ...
  const { addOrder } = useInventoryAlerts()
  ...
  const fetchMedicines = useCallback(
    async (page) => {
      ...
      const items = result.items.map((item) => {
        const mapped = mapMedicineToFe(item, [])
        return {
          ...mapped,
          price: Number(mapped.listPrice || mapped.salePrice || mapped.price || 0),
          category: mapped.category || 'Chưa phân nhóm',
          ingredient: mapped.ingredient || 'Chưa cập nhật',
          usage: mapped.usage || 'Chưa cập nhật',
        }
      })
    ...
    setCart((prev) =>
      prev.map((item) => {
        if (item.id === id) {
          ...
          if (parsed > Number(item.stock || 0)) {
            showAlert(
              'Không đủ tồn kho',
              `${item.name} chỉ còn ${item.stock} ${item.unit} trong kho.`,
            )
            return item
          }
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const realInvoiceId = created?.invoiceId || '—'
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 xl:grid-cols-4">
        ...
        <div className="space-y-4 lg:col-span-2 xl:col-span-3">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-4">
            {loading ? (
              <div className="col-span-full py-10 text-center text-slate-400">Đang tải sản phẩm...</div>
            ) : medicines.length > 0 ? (
              medicines.map((med) => {
                const availableStock = getAvailableStock(med.id, med.stock)
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md overflow-hidden rounded-[28px] bg-white shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6">
              ...
              {(() => {
                const availableStock = getAvailableStock(selectedMedicine.id, selectedMedicine.stock)
                const canAdd = availableStock > 0 && selectedMedicine.status !== 'INACTIVE'
                return (
                  <button
                    type="button"
                    onClick={() => handleAddToCart(selectedMedicine)}
                    disabled={!canAdd}
                    className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-blue-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-blue-600/30 transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-300 disabled:shadow-none"
                  >
                    <FaCartPlus size={16} />
                    {canAdd ? 'Thêm vào giỏ' : 'Không thể thêm (hết hàng / ngừng bán)'}
                  </button>
                )
              })()}
            </div>
