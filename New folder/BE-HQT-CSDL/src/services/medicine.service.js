...
// API
// ============================================================
const CURRENT_STOCK_SQL = `ISNULL((
  SELECT SUM(b.CurrentQty)
  FROM dbo.MedicineBatch b
  WHERE b.MedicineId = m.MedicineId
    AND b.ExpiryDate >= CAST(GETDATE() AS DATE)
...
function buildMedicineFilters(filters = {}) {
  ...
  if (filters.categoryName) {
    conditions.push('c.CategoryName = @categoryName')
  ...
  if (filters.stockStatus === 'normal') {
    conditions.push(`${CURRENT_STOCK_SQL} > m.MinStock`)
  }
...
function buildMedicineOrderBy(filters = {}) {
  if (filters.sortBy === 'stock') {
    return `${CURRENT_STOCK_SQL} ${filters.sortOrder === 'desc' ? 'DESC' : 'ASC'}, m.MedicineName`
  }
...
export async function getAll(filters = {}) {
  ...
  const limit = Number(filters.limit) || total || 25

  const orderBy = buildMedicineOrderBy(filters)
  const rows = await query(
    `${SELECT_MEDICINE} ${where}
     ORDER BY ${orderBy}
...
export async function getInventoryStats() {
  const row = await queryOne(`
    SELECT
      (SELECT COUNT(*) FROM dbo.Medicine WHERE IsActive = 1) AS totalMedicines,
      (SELECT COUNT(*)
       FROM dbo.Medicine m
       WHERE m.IsActive = 1
         AND ISNULL((
...
export async function updateBatch(medicineId, batchId, data) {
  const batch = await queryOne(
    `SELECT BatchId, MedicineId, ImportPrice, ImportDate, ExpiryDate, InitialQty, CurrentQty
     FROM dbo.MedicineBatch WITH (UPDLOCK, ROWLOCK)
     WHERE BatchId = @batchId AND MedicineId = @medicineId`,
    { batchId, medicineId },
  )
  if (!batch) {
    throw new AppError('Không tìm thấy lô hàng', 404, 'BATCH_NOT_FOUND')
  }

  const soldQty = Number(batch.InitialQty) - Number(batch.CurrentQty)
  const nextCurrentQty =
    data.currentQty !== undefined ? Number(data.currentQty) : Number(batch.CurrentQty)
  const nextImportPrice =
    data.importPrice !== undefined ? Number(data.importPrice) : Number(batch.ImportPrice)
  const nextExpiryDate = data.expiryDate !== undefined ? data.expiryDate : batch.ExpiryDate

  if (!Number.isInteger(nextCurrentQty) || nextCurrentQty < 0) {
    throw new AppError('Số lượng lô phải là số nguyên không âm', 400, 'INVALID_BATCH_QTY')
  }
  if (nextImportPrice < 0) {
    throw new AppError('Giá nhập phải >= 0', 400, 'INVALID_IMPORT_PRICE')
  }
  if (!nextExpiryDate) {
    throw new AppError('Hạn sử dụng không hợp lệ', 400, 'INVALID_EXPIRY_DATE')
  }

  const importDate = batch.ImportDate instanceof Date
    ? batch.ImportDate.toISOString().slice(0, 10)
    : String(batch.ImportDate).slice(0, 10)
  const expiryDate = String(nextExpiryDate).slice(0, 10)
  if (expiryDate < importDate) {
    throw new AppError('Hạn sử dụng phải sau ngày nhập', 400, 'EXPIRY_BEFORE_IMPORT')
  }

  const nextInitialQty = nextCurrentQty + soldQty
