...
export async function getAll(filters = {}) {
  ...
  if (filters.search) {
    conditions.push(
      `(CustomerName LIKE N'%' + @search + N'%' OR Phone LIKE '%' + @search + '%' OR CustomerId LIKE '%' + @search + '%')`,
    )
  ...
  const countRow = await queryOne(
    `SELECT COUNT(*) AS total FROM dbo.Customer WHERE ${where}`,
    params,
  )
...
export async function getInvoices(customerId, pagination = {}) {
  ...
  const countRow = await queryOne(
    `SELECT COUNT(*) AS total FROM dbo.SalesInvoice WHERE CustomerId = @id`,
    { id: customerId },
  )
  const total = Number(countRow?.total || 0)
  const offset = Number(pagination.offset) || 0
  const limit = Number(pagination.limit) || total || 25
