...
export async function getAll(filters = {}) {
  ...
  if (filters.search) {
    conditions.push(
      `(e.FullName LIKE N'%' + @search + N'%' OR e.Username LIKE N'%' + @search + N'%' OR e.EmployeeId LIKE N'%' + @search + N'%' OR e.Phone LIKE N'%' + @search + N'%')`,
    )
    params.search = filters.search
  }
  if (filters.roleId) {
    conditions.push('e.RoleId = @roleId')
  ...
  const countRow = await queryOne(
    `SELECT COUNT(*) AS total FROM dbo.Employee e JOIN dbo.Role r ON r.RoleId = e.RoleId ${where}`,
    params,
  )
  ...
  const summaryRow = await queryOne(
    `SELECT
       ...
       SUM(CASE WHEN e.RoleId = 'ADMIN' THEN 1 ELSE 0 END) AS adminCount,
  ...
  return {
    ...
    summary: {
      ...
      adminCount: Number(summaryRow?.adminCount || 0),
