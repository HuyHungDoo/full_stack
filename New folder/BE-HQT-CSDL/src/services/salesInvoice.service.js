...
import { vnDateSql } from '../utils/vnDateSql.js'
...
export async function getAll(filters = {}, requestUser) {
  ...
  if (filters.from) {
    conditions.push(`${vnDateSql('i.InvoiceDate')} >= @fromDate`)
    params.fromDate = filters.from
  }
  if (filters.to) {
    conditions.push(`${vnDateSql('i.InvoiceDate')} <= @toDate`)
  ...
  if (filters.search) {
    conditions.push(
      `(i.InvoiceId LIKE N'%' + @search + N'%' OR i.CustomerNameSnapshot LIKE N'%' + @search + N'%' OR i.PhoneSnapshot LIKE N'%' + @search + N'%')`,
    )
  ...
  const countRow = await queryOne(
    `SELECT COUNT(*) AS total FROM dbo.SalesInvoice i ${where}`,
    params,
  )
  ...
  `
  const rows = await query(sql, { ...params, offset, limit })
  return {
